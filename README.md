...
gulp -ws
```
运行`gulp -ws`命令，会监听`src`目录下所有文件的变更，并且默认会在`8080`端口启动一个express服务器，然后在浏览器打开 [http://localhost:8080/example](http://localhost:8080/example)。
...
`Cell`，列表视图，用于将信息以列表的结构显示在页面上，是wap上最常用的内容结构。`Cell`由多个section组成，每个section包括section header`cells_title`以及table view cells`cells`。

`cell`由thumnail`cell_hd`、body`cell_bd`、accessory`cell_ft`三部分组成，`cell`采用自适应布局，在需要自适应的部分加上class`cell_primary`即可：
